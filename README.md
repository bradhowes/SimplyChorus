...
`SimplyChorus` in them will be replaced with the first argument given to `build.py` (e.g. "MyEffect"), and all text
files will be changed so that the strings `SimplyChorus` and `chor` are replaced with their respective substitutions
that you provided.
